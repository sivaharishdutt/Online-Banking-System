...
void Menu(int );
void displaySubMenu(int, struct database,struct flock,int);
void adminFunctions(int);
void deleteAccount(int);
void balance(int,struct database,struct flock,int);
void deleteJointAccount(int);
void jointDisplaySubMenu(int, struct jointdatabase,struct flock,int );
...
void addjointAccount(char uname1[], char pass1[], char uname2[], char pass2[], int sd)
{
	...
	db.acc_id=(rand()%1001)+8000;

	write(fd1, (char *)&db, sizeof(struct jointdatabase));

	write(sd, "Account Created",20);
...
void login(int sd)
{
		...
		if(!strcmp(db.uname, uname) && !strcmp(db.pass, pass))
			{
				...
				write(sd, "Single User Login Successful", sizeof("Single User Login Successful"));
				displaySubMenu(sd,db,lock,fd1);
	...
	flag=0;
	write(sd, &flag, sizeof(flag));
	write(sd, "Invalid Credentials", 24);
	Menu(sd);	
}
	...
	while(read(fd1, (char *)&db, sizeof(struct jointdatabase)))
	{
		...
		if((!strcmp(db.uname1, uname1) && !strcmp(db.pass1,pass) && !strcmp(db.uname2, uname2)) || (!strcmp(db.uname1, uname2) && !strcmp(db.uname2, uname1) && !strcmp(db.pass2,pass)))
			...
			/*Locking*/	
				...
				write(sd, &flag, sizeof(flag));
				write(sd, "Login Successful", 23);
				jointDisplaySubMenu(sd,db,lock,fd1);
...
void balance(int sd,struct database db,struct flock lock,int fd)
{
	write(sd,db.uname,sizeof(db.uname));
	write(sd,&db.balance,sizeof(db.balance)); 
	displaySubMenu(sd,db,lock,fd);
}

void viewDetails(int sd, struct database db,struct flock lock,int fd)
{
	write(sd,db.uname, sizeof(db.uname));
...
void viewJointDetails(int sd, struct jointdatabase db,struct flock lock,int fd)
{
	...
	write(sd,&db.acc_id, sizeof(db.acc_id));
	write(sd,&db.balance, sizeof(db.balance));
	jointDisplaySubMenu(sd,db,lock,fd);
}

void deposit(int sd, struct database db,struct flock lock,int fd)
{
	float amount;
...
void withdraw(int sd, struct database db,struct flock lock,int fd)
{
	int   flag=0;
	float amount;
...
void passChange(int sd, struct database db,struct flock lock)
{
	char pass[MAX];
	read(sd,pass,sizeof(pass));
	strcpy(db.pass,pass);
...
void displaySubMenu(int sd, struct database db,struct flock lock,int fd)
{
	int choice;
	read(sd, &choice, sizeof(choice));
	switch(choice)
	{
		case 1: balance(sd,db,lock,fd);
				break;
		case 2: viewDetails(sd,db,lock,fd);
				break;
		case 3: deposit(sd,db,lock,fd);
				break;
		case 4: withdraw(sd,db,lock,fd);
				break;
		case 5: passChange(sd,db,lock);
				break;
...
void jointDisplaySubMenu(int sd,struct jointdatabase db,struct flock lock,int fd)
{
	int choice;
	read(sd, &choice, sizeof(choice));
	switch(choice)
...
void adminFunctions(int sd)
{
	...
	switch(choice)
	{
		...
		case 3:read(sd, uname, sizeof(uname));
				if(checkAccountExists(uname))
			{
				write(sd, "Account Found\0", 19);
			}
			else
			{
				write(sd, "Account does not exist with the given Username", 51);
			}
			adminFunctions(sd);
			break;
...
void deleteAccount(int sd)
{
	...
	if(!checkAccountExists(uname))
	{
		write(sd, "Account does not exist", 27);
	}
	else
	{	
		...
		write(sd,"Account deleted Successfully", 33);
...
void adminLogin(int sd)
{
	...
	{	
		flag=0;
		write(sd, &flag, sizeof(flag));
		write(sd,"Invalid Credentials", 25);
...
int main()
{
	...
	int sd, client_addr_len, nsd,binde;
	...
	binde=bind(sd,(struct sockaddr *)(&server), sizeof(server));
	if(binde==-1)
	{
		perror("\nError when binding\n");
	}
